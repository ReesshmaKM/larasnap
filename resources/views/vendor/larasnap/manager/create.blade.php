@extends('larasnap::layouts.app', ['class' => 'manager-create'])
...
<div class="row">
   <div class="col-xl-12">
      <div class="card shadow mb-4">
         <div class="card-body">
            <div class="card-body">
               <a href="{{ route('managers.index') }}" title="Back to Manager List" class="btn btn-warning btn-sm"><i aria-hidden="true" class="fa fa-arrow-left"></i> Back to Manager List
               </a> 
               <br> <br> 
            <form method="POST" action="{{ route('managers.store') }}"  enctype="multipart/form-data" class="form-horizontal" autocomplete="off">
			   @csrf
                <div class="row">
					<div class="col-md-4">
					...
					<div class="col-md-4">
						<div class="form-group">
							<label for="mobile_phone" class="control-label">Mobile No<small class="text-danger required">*</small></label> 
							<input name="mobile_phone" type="number" id="mobile_phone" class="form-control" min="0" value="{{ old('mobile_phone') }}">
							@error('mobile_phone')
							 <span class="text-danger">{{ $message }}</span>
							@enderror 
						</div>
